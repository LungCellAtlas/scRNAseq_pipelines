...
# use force to delete existing refgenome folder
force="false"
...
where:
    ...
    -d  download: whether or not to include download of the needed files from ensembl. This can be set to false if the script was run before, but was halted after the downloads, e.g. at the cellranger mkref command. (default: ${download})
    -u  force: if set to true this will remove existing log file and index folder and run the indexing without the parameter check by the user. This argument is ignored if -d is set to false (default: ${force})
    -o  output directory: directory in which to store reference genome"
    

while getopts ':h:e:c:t:m:g:s:f:a:n:y:x:u:o:d:' option; do
  case "$option" in
    h) echo "$usage"
       exit
       ;;
...
# Check if outdir already exists: it shouldn't.
if [ -d ${outfolder} ]; then  
  if [[ ${force} != "true" ]] 
    then
  ...
  echo "Warn: Output directory ${outfolder} exists but is removed, since force is used."
  rm -r ${outfolder}
fi
...
if [[ ${force} != "true" ]] && [[ ${download} == "true" ]]
  ...
  echo "Parameter checking is skipped, since force is used or since -d is set to false."
fi
...
# only perform downloads if download is set to true
