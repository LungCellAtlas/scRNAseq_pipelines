...
### setting up:  
The first script, [LCA_pipeline_setup.sh](LCA_pipeline_setup.sh), will download the files required to set up your conda environment (including cellranger), and the files to test the pipeline. It will then create the required conda environment with set versioning of the needed software, and finally it will build a reference genome. It might take a few hours (about 6 for us) to run the entire script.
...
- w: path to working directory. Testdata, reference genome, log files etc. will be stored here.
...
- e: ensembl release
- g: genome release  
(These 5 flags all have defaults, check the documentation to see the defaults.)

Moreover, the script is modular and it is possible to exclude parts, e.g. if they were already executed in a previous run. By default, all are set to true.
...
- L: when true, include downloads of necessary files from ensembl to build reference genome
...
```
$ ./LCA_pipeline_setup.sh -w /users/lisa.sikkema/LCA_scRNAseq_pipeline -u 1234:5678 -c /users/lisa.sikkema/miniconda3/envs/  
```
...
- p: "profile" for computation; either 'local' or 'cluster'. If set to 'cluster', a job will be submitted to your cluster from within the script. For further details, check documentation under -h
...
```
$ ./LCA_pipeline_testrun.sh -p cluster -e /users/lisa.sikkema/miniconda3/envs/cr3-velocyto-scanpy -s HELMHOLTZ -u true -l https://fake/example/link -o /users/lisa.sikkema/LCA_scRNAseq_output/testrun -w /users/lisa.sikkema/LCA_scRNAseq_pipeline -c 20 -m 90 -t 10 -q hh_cpu -C 'nice=10000 -t 2:00:00 --qos=hh_stndrd'
``` 

### run the pipeline on your data

Once you have completed the testrun successfully, you can run the pipeline on your own data. This is similar to doing the testrun, except now you'll need to create a tsv (or .xls or tab-separated .txt) file with information about the samples you want to run the pipeline on. An example with one sample is shown [here](res/Samples_file_example.xls). The table should be a tab-separated file containing the samples, one line per sample. The sample header must consist of the following fields.  
...
An example of such is created when running the LCA_pipeline_testrun.sh script, and can be found in the thus created testrun_v[pipeline_version] folder, under "Samples_testdata_testrun.txt".

When your table is ready, you can run the [LCA_pipeline_run.sh](LCA_pipeline_run.sh) script with only one line of code to run the pipeline on your samples. Detailed documentation on how to use the command can be found under  
```
$ ./LCA_pipeline_run -h
```  
We will give a brief outline of the arguments here as well, partly overlapping with the testrun. Mandatory arguments are:

- p: "profile" for computation; either 'local' or 'cluster'. If you'd like a job to be submitted to your cluster from within the script, use "cluster". For further details, check documentation under -h
- e: path to conda environment that was created during pipeline setup, it should have the name "cr3-velocyto-scanpy"
- s: sitename (name of site where data was generated, e.g. SANGER or HELMHOLTZ)
...
An example of a full command to use is:  
```
$ ./LCA_pipeline_run.sh -p local -e /users/lisa.sikkema/miniconda3/envs/cr3-velocyto-scanpy -s HELMHOLTZ -n lung_A1_cohort -x /users/lisa.sikkema/LCA_scRNAseq_output/lung_dataset/Samples_testdata_fullrun.txt -u true -l https://fake/upload/link -o /users/lisa.sikkema/LCA_scRNAseq_output/lung_dataset -c 22 -m 80 -t 11  
``` 
